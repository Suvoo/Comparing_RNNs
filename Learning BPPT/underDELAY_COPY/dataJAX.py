...
def genData(key, length, width, initial_delay, initial_delay_fixed_length, delay, 
delay_fixed_length, batch_size, binary_encoding, blank_symbol):
    ...
    str_ = jnp.unpackbits(jrandom.choice(key, np.array(range(1, 2 ** 3)) ,(length,))
    .astype(np.uint8).reshape(-1, 1), axis = 1)[:, -tmp_width:]

    if width > 8:
        str_2 = jnp.unpackbits(jrandom.choice(key, np.array(range(0, 2 ** (width - tmp_width))), (length,)).astype(np.uint8).reshape(-1, 1), axis=1)[:, -(width - tmp_width):]
...
def custom(batch_size,key):
    # a = np.array([])
    # a = np.empty([batch_size])
    a = []
    for i in range(batch_size):
        # key, subkey = jrandom.split(key)
        e = genData(key, length, width, initial_delay, initial_delay_fixed_length, delay, 
    delay_fixed_length, batch_size, binary_encoding, blank_symbol)
        a.append(e['x'])
        # np.insert(a,e['x'])
        # np.append(a,e['x'])
